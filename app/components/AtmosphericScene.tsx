  ...
  return (
    <group ref={rig}>
      <Points ref={points} positions={positions} stride={3} frustumCulled>
        <PointMaterial transparent color="#d9ff7d" size={0.018} sizeAttenuation depthWrite={false} opacity={0.72} />
      </Points>
      <Line points={pathway} color="#a8e8ca" lineWidth={0.65} transparent opacity={0.35} />
      {pathway.map((point, index) => (
        <Float key={index} speed={0.7 + index * 0.12} rotationIntensity={0.1} floatIntensity={0.3}>
          <mesh position={point}>
            ...
            <meshBasicMaterial color={index === 3 ? "#d9ff7d" : "#8ac9ae"} transparent opacity={0.75} />
          </mesh>
        </Float>
      ))}
    </group>
